...
nodething = gpd.read_file(os.path.join(args.city, "pano_metadata_s1.geojson"))
dateslist = [ datetime.fromtimestamp(x / 1000.0) for x in nodething.captured_at ]
nodething['pano_date'] = [ '-'.join([str(x.year), str(x.month)]) for x in dateslist ]

nodething = nodething[['pano_id','pano_date','geometry']]
nodething['error'] = np.repeat([False], len(nodething))
nodething['filepath'] = [ os.path.join(args.city, "panoimages", "s1",  x + '.png') for x in nodething.pano_id ]
